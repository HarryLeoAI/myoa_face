...
- 重点在于:
  1. 点击事件打开表单时, 对已经获取的所有用户进行再次筛选, 根据点击事件传入的部门:一是筛选出该部门下辖员工作为leader候选人(渲染在leader.select中), 二是筛选出董事会下辖员工, 作为manager候选人
  2. select 下拉框绑定了值`v-model="departmentFormData.manager"`之后, 表单页面在打开时会自动选择到指定的option
  3. 模板部分, 使用`el-row`包`el-col`轻松实现多宫格样式的表格,值得收藏
- 关于`staffHttp.js`里新增的两个方法: 获取不分页的员工数据(所有员工) 以及修改部门信息, 实在写臭了,略
